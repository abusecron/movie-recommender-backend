...
item_factors = np.nan_to_num(item_factors.astype(np.float32), nan=0.0, posinf=0.0, neginf=0.0)

content_matrix = joblib.load(f"{ARTIFACTS_PATH}/content_matrix.pkl")
content_matrix = np.asarray(content_matrix, dtype=np.float32)
...
# Build content_by_enc using vectorized mapping
print("Building content lookup...")
enc_classes = film_enc.classes_
content_by_enc = np.zeros((len(enc_classes), content_matrix.shape[1]), dtype=np.float32)
for i, fid in enumerate(enc_classes):
    pos = film_id_to_pos.get(fid)
    if pos is not None:
        content_by_enc[i] = content_matrix[pos]
...
def recommend_for_new_user(liked_ids, disliked_ids, n=10):
    ...
    svd_scores = np.zeros(item_factors.shape[0], dtype=np.float32)
    if liked_idxs:
    ...
    content_scores = np.zeros(item_factors.shape[0], dtype=np.float32)
    if liked_idxs:
